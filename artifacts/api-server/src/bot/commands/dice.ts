...
function drawDiceImage(
  ...
  const background = ctx.createLinearGradient(0, 0, 0, IMAGE_HEIGHT);
  ...
  ctx.lineWidth = 5;
  roundedRect(ctx, 2.5, 2.5, IMAGE_WIDTH - 5, IMAGE_HEIGHT - 5, 18);
  ctx.stroke();

  ctx.fillStyle = "#b9c5d8";
  ctx.font = "700 14px Arial";
  ctx.letterSpacing = "5px";
  ctx.fillText("DICE", 48, 45);
  ...
  ctx.strokeText(roll.toFixed(2), 48, 128);
  ctx.fillText(roll.toFixed(2), 48, 128);

  const chance = direction === "over"
    ? 100 - target
    : target;
  const targetLabel = `${direction.toUpperCase()} ${target}`;
  const chanceLabel = `${chance}% WIN CHANCE`;

  ctx.textAlign = "right";
  ctx.fillStyle = "#ffd34d";
  ctx.font = "900 33px Arial";
  ctx.fillText(targetLabel, IMAGE_WIDTH - 43, 87);
  ctx.fillStyle = "#aebbd0";
  ctx.font = "700 20px Arial";
  ctx.fillText(chanceLabel, IMAGE_WIDTH - 43, 117);
  ctx.textAlign = "left";

  ctx.fillStyle = "#101827";
  ctx.strokeStyle = "#050b15";
  ctx.lineWidth = 5;
  roundedRect(ctx, 42, 178, 940, 72, 18);
  ctx.fill();
  ctx.stroke();

  const trackX = BAR_X;
  const trackY = BAR_Y;
  const split = trackX + (target / 100) * BAR_WIDTH;
  const winningLeft = direction === "under";

  ctx.save();
  roundedRect(ctx, trackX, trackY, BAR_WIDTH, BAR_HEIGHT, 14);
  ctx.clip();
  ctx.fillStyle = winningLeft ? "#2cae6b" : "#e34d5b";
  ctx.fillRect(trackX, trackY, split - trackX, BAR_HEIGHT);
  ctx.fillStyle = winningLeft ? "#e34d5b" : "#2cae6b";
  ctx.fillRect(split, trackY, trackX + BAR_WIDTH - split, BAR_HEIGHT);
  ctx.restore();

  ctx.globalAlpha = 0.18;
  ctx.strokeStyle = "#ffffff";
  ctx.lineWidth = 3;
  for (let x = trackX - 20; x < trackX + BAR_WIDTH; x += 36) {
    ctx.beginPath();
    ctx.moveTo(x, trackY);
    ctx.lineTo(x + 30, trackY + BAR_HEIGHT);
    ctx.stroke();
  }
  ctx.globalAlpha = 1;
  ...
  drawDie(
    ctx,
    Math.max(trackX + 22, Math.min(trackX + BAR_WIDTH - 22, rollX)),
    trackY + BAR_HEIGHT / 2,
    Math.max(1, Math.min(6, Math.floor(roll) % 6 + 1)),
  );

  ctx.fillStyle = "#8b98ad";
  ctx.font = "700 15px Arial";
  ctx.textAlign = "center";
  ctx.fillText("BUST", IMAGE_WIDTH / 2, 274);
  ctx.font = "700 16px Arial";
  for (const tick of [0, 25, 50, 75, 100]) {
    const tickX = trackX + (tick / 100) * BAR_WIDTH;
    ctx.fillText(String(tick), tickX, 300);
  }
...
function resultContainer(
  ...
  return new ContainerBuilder()
    .setAccentColor(won ? COLORS.success : COLORS.danger)
    .addTextDisplayComponents(
      text("## 🎲 Dice"),
    )
    .addTextDisplayComponents(
      text(won ? "### ✅ You won" : "### ❌ Roll missed"),
    )
    ...
    .addTextDisplayComponents(
      text(
        [
          ...
          `🎯 **Roll**  \`${roll.toFixed(2)}\` · ${direction} ${target}`,
...
export async function execute(
  ...
  if (!Number.isInteger(target) || target < 10 || target > 90) {
    return void interaction.reply({
      embeds: [
        errorEmbed("Target must be a whole number from **10** to **90**."),
      ],
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  await addBalance(interaction.user.id, netDelta);
  await recordBet(
    interaction.user.id,
    amount,
    netDelta,
  ...
  const attachment = new AttachmentBuilder(
    drawDiceImage(roll, target, direction),
  ).setName(filename);
